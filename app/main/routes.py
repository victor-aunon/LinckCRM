...
from app.models import User, MyCompany, PermissionsEnum, Message, Company
# from app.translate import translate

# g, current_app and current_user work like global variables,
# but are only accessible during the handling of a request,
# and only in the thread that is handling it.
...
def index():
    ...
    db.session.commit()
    # form = MessageForm()
    users = MyCompany.query.filter_by(id=current_user.company_id).first().users
    ...
    return render_template('index.html', title=_('Home Page'), users=users,
...
def show_my_company():
    ...
    if current_user.permissions.value not in ['Admin', 'Owner']:
